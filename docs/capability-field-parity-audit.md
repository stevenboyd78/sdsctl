...
| GST/PWF/GWF waterfall | Exact typed GST plus variable PWF fields and exactly 240 uninterpreted GWF values, including the physically observed terminal separator; one demand-driven radio/daemon-owned, recurring-get session and private bounded local fanout | Authenticated demand-driven web spectrum and rolling-waterfall Canvas with semantic lifecycle/loss telemetry and preserved raw strings; bounded validating daemon-client diagnostic | `S`, `F`, `P`; SDS200 firmware 1.26.01 LAN qualification covers one-frame GWF gets, overlapping clients, reconnect, restart, cleanup, and normal-mode restoration; the web renderer remains explicitly relative and uncalibrated; FFT magnitude/calibration and other model/firmware semantics remain unknown | R3 |
