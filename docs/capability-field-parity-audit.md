...
| GST/PWF/GWF waterfall | Exact typed GST plus variable PWF fields and exactly 240 uninterpreted GWF values; one demand-driven radio/daemon-owned session and private bounded local fanout | Bounded validating daemon-client diagnostic only; product rendering is deferred to Milestone 27.3 | `S`, `F`, `I`; lifecycle ownership is implemented, but physical SDS200 qualification and FFT semantics remain incomplete | R1/R3 |
