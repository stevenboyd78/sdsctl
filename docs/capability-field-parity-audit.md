...
| GST/PWF/GWF waterfall | Exact typed GST plus variable PWF fields and exactly 240 uninterpreted GWF values, including the physically observed terminal separator; one demand-driven radio/daemon-owned, recurring-get session and private bounded local fanout | Bounded validating daemon-client diagnostic only; the responsive web renderer is Milestone 27.4 | `S`, `F`, `P`; SDS200 firmware 1.26.01 LAN qualification covers one-frame GWF gets, overlapping clients, reconnect, restart, cleanup, and normal-mode restoration; FFT magnitude/calibration and other model/firmware semantics remain unknown | R1/R3 |
