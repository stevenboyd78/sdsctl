...
def test_roadmap_records_milestone_26_8_semantic_control_contract() -> None:
    ...
    for required in (
        ...
        "volume/squelch portion of the physical matrix did not pass on SDS200 UDP",
        "timed out without changing the authoritative starting values",
        "documented `V`/`Q` knob keys plus one rotary step returned `KEY,OK`",
        "No local USB scanner was available for serial comparison",
        "must not claim physical SDS200 UDP level mutation",
