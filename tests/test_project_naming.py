...
def test_roadmap_records_milestone_26_12_modular_tui_theme_contract() -> None:
    ...
    for required in (
        ...
        "Milestone 26.11 is closed",
        "`themes/tui/<theme-name>/`",
        "stable identities `dark` and `light`",
        "semantic terminal palette",
        "theme-only Textual CSS",
        "versioned declarative `manifest.json`",
        "complete renderer-neutral `palette.json`",
        "one local `theme.tcss`",
        "stable lowercase kebab-case identity",
        "directory identity",
        "optional existing Textual screen class",
        "typed immutable registry",
        "reject duplicate identities, orders, palette names, or screen classes",
        "missing or undeclared files",
        "traversal, absolute, or remote",
        "invalid style values",
        "missing or unknown semantic roles",
        "Replace the Python-literal built-in palette definitions",
        "preserving `DEFAULT_DARK_THEME`, `DEFAULT_LIGHT_THEME`",
        "palette names `default-dark` and `default-light`",
        "same singleton palette objects",
        "Move only theme-owned Textual colors, backgrounds, and borders",
        "Keep shared dimensions, padding, scrolling",
        "existing `light` screen class",
        "dark remains the default",
        "plain-text/JSON output must not depend on color",
        "exact semantic-role completeness",
        "compatibility object identity",
        "live TUI theme toggling",
        "installed-wheel resource loading",
        "No physical scanner revalidation is required",
        "continued shared use of the two semantic palettes by Rich CLI and Textual",
        "Do not enable third-party discovery",
        "new theme selection values",
        "Do not modify the web or Home Assistant registries",
        "add a GUI",
